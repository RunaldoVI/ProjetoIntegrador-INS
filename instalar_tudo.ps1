...
if (-not $wslVersion) {
    ...
    Write-Host '⏳ A instalação do WSL pode demorar...'
    Write-Host '⚠️ Quando terminares a criação da conta Linux, volta a correr este script.'
    pause
    exit
} else {
...
if ($distros -notmatch 'Ubuntu-22.04') {
    ...
    wsl --install -d Ubuntu-22.04
    Write-Host '⏳ A configuração da conta Linux pode demorar...'
    Write-Host '⚠️ Quando terminares, volta a correr este script.'
    pause
    exit
} else {
